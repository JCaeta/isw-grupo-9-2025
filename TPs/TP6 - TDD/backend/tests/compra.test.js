...
describe('Compra de entradas - Grupo 9', () => {
  ...
  test('Verifica el cálculo de precios', () => {
  ...
  test('No puede comprar lunes o día festivo (día cerrado)', () => {
    const usuario = new Usuario('Nacho', true);
  ...
  test('No puede comprar más de 10 entradas', () => {
    ...
    expect(
      () => new Compra(usuario, new Date(2025, 10, 8, 10, 0), entradas, 'tarjeta'),
    ).toThrow('Máximo 10');
  });
  ...
  test('Usuario no registrado', () => {
    ...
    expect(
      () => new Compra(usuario, new Date(2025, 10, 8, 10, 0), entradas, 'efectivo'),
    ).toThrow('registrado');
  });
  ...
  test('Pago con tarjeta incluye mensaje de confirmación, cantidad, fecha y envía email', async () => {
    const usuario = new Usuario('Nacho', true);
    const entradas = [
      new Entrada(30, 'VIP'),
    ...
    expect(result.redirigidoA).toBe('Mercado Pago');
  });
});
